...
ChatLogic::ChatLogic()
{
    ...
    _chatBot->SetChatLogicHandle(this); // this refers to chatlogic
    */
...
ChatLogic::~ChatLogic()
{
    ...
    // delete chatbot instance: now managed by graphnode
    //delete _chatBot;
    ...
    for (auto& it : _nodes)
    {
        //std::cout << "Delete one node " << it << std::endl;
        it = nullptr;
    }

    // delete all edges: now managed by graphnode
    /*
                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "EDGE")
                    {
                        ...
                        if (parentToken != tokens.end() && childToken != tokens.end())
                        {
                            ...
                            _edges.push_back(edge); // vector of all edges

                            // find all keywords for current node
                            AddAllTokensToElement("KEYWORD", tokens, *edge);

                            // store reference in child node and parent node
                            (*childNode)->AddEdgeToParentNode(edge); // non-owning
                            (*parentNode)->AddEdgeToChildNode(edge); // owning
                        }
